...
Custom animation easing is a great way to customize the motion ethos in a design system. 
These easing props are one of the best augmentation packages for **existing** design systems, 
bringing motion tokens to systems that haven't gotten there yet.

:::tip
Each easing set offers **5 strengths**. 
:::
...
Get the props into your project via [NPM](https://www.npmjs.com/package/open-props) or [CDN](https://unpkg.com/open-props/).
...
<Tabs>
  ...
  <TabItem label="JS">
		...
		export const ElasticIn = [
			...
			'cubic-bezier(.5, -1.00, .75, 1)',
			'cubic-bezier(.5, -1.25, .75, 1)',
		]
		```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="JS">
		...
		export const ElasticInOut = [
			...
			'cubic-bezier(.5, -.7, .1, 1.5)',
			'cubic-bezier(.5, -.9, .1, 1.5)',
		]
		```
  </TabItem>
</Tabs>
...
<Easings ease="ease-spring" client:visible />
...
<Easings ease="ease-bounce" duration="3s" client:visible />
